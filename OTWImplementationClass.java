 ...
 * @Modified by:
 */
public class OTWImplementationClass {
	...
	// EventDetails ed1 = new EventDetails(4, booklist1);
    ...
    public void addEventHelper(HashMap<String, EventDetails> curList, String userID, String eventID, int eventType,
	    int bookingCapacity) {
	EventDetails cur;
	otwLogRecorder.writeNorLog(otwLogRecorder.paraConstructor(userID, "", eventType, eventID, bookingCapacity),
	...
	if (curList.containsKey(eventID)) {
	    curList.remove(eventID);
	    otwLogRecorder.writeNorLog(otwLogRecorder.paraConstructor(userID, "", eventType, eventID, 0),
	...
	otwLogRecorder.writeNorLog(otwLogRecorder.paraConstructor(userID, "", eventType, "", 0),
	...
	if (tripleCheck == -1) {
	    ...
	    if (rs == 0)
		otwLogRecorder.writeNorLog(otwLogRecorder.paraConstructor("", customerID, eventType, eventID, 0),
	...
	otwLogRecorder.writeNorLog(otwLogRecorder.paraConstructor("", customerID, 4, "", 0),
	...
	if (checkRs == -3) {
	    otwLogRecorder.writeNorLog(otwLogRecorder.paraConstructor("", customerID, newEventType, newEventID, 0),

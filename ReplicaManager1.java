...
public class ReplicaManager1 {
	...
	public void solveCrash(String msg) {
		int crashRM = Integer.parseInt(msg.split(":")[1]);
		if (crashRM == replicaID) {
			...
			case 1:
				sendCrashToRM(RMPort.RM_PORT.rmHost1, RMPort.RM_PORT.rmPort1_failure, crashInfo);
				break;
			case 2:
				sendCrashToRM(RMPort.RM_PORT.rmHost2, RMPort.RM_PORT.rmPort2_failure, crashInfo);
				break;
			case 3:
				sendCrashToRM(RMPort.RM_PORT.rmHost3, RMPort.RM_PORT.rmPort3_failure, crashInfo);
				break;
			case 4:
				sendCrashToRM(RMPort.RM_PORT.rmHost4, RMPort.RM_PORT.rmPort4_failure, crashInfo);
				break;
			}
		}
	}
	...
	private void sendCrashToRM(String RMAddress, int RMFailurePort, String msg) {
		DatagramPacket reply = null;
		DatagramSocket socket = null;
		String crashConfirm = "";
		try {
			...
			DatagramPacket packet = new DatagramPacket(data, data.length, address, RMFailurePort);
			...
			if (crashConfirm.equals("DidCrash")) {
				...
				DatagramPacket restartPacket = new DatagramPacket(data2, data2.length, address, RMFailurePort);
	...
	public void RMListener(int RMPort) throws Exception {
		...
		while (true) {
			...
			case "Failure":
				...
				int needFix = SetCounter(rid);// Failure:seqID:replicaId
				if (needFix == -1) {
					if (rid == replicaID) {
						//fix
						System.out.println("Replica " + rid + " has fixed the failure");
					}
				}
				break;
			case "Crash":
				solveCrash(message); // Crash:seqID:replicaId
				break;

...
//		logger.info("restart and recover replica1.");
	...
	private void sendCrashToRM(String RMAddress, int RMFailurePort, String crashMsg) {
		DatagramPacket reply = null;
		int send_count = 0;
		boolean revResponse = false;
		DatagramSocket socket = null;
	...
	public void RMListener(int RMPort) throws Exception {
		...
		while (true) {
			...
			case "Failure":
				...
				if (needFix == -1) {
					System.out.println("Replica " + replicaID + " has fixed the failure");
				}
				break;
			case "Crash":
				solveCrash(message); // Crash:seqID:replicaId (if choose crash
				break;
			case "SetCrash":// SetCrash:replicaId
				if(messageSplited[1].equals(replicaID+"")) {
					replica1.crash = true;
				}
				break;
			default:

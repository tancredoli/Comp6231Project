...
import EventRecord.Event;
import ServerApp.Server_interface;
import ServerApp.Server_interfaceHelper;
import ServerApp.Server_interfacePOA;

public class Server_MTL extends Server_interfacePOA {
	// database of mtl
	...
	public Server_MTL() throws Exception {
		super();
	}
		...
		if (!customerID.substring(0, 3).equalsIgnoreCase(config_MTL.SERVER_NAME)) {
			...
			if (checkCustomerID(customerID).equalsIgnoreCase("invalid")) {
				...
				if(flag_cancel) {
					...
					if (flag == 1 || flag == -2 && !old_city.equalsIgnoreCase(config_MTL.SERVER_NAME) && !new_city.equalsIgnoreCase(config_MTL.SERVER_NAME)
						...
						if (isLocalEvent(newEventID)) {
							...
							s += sendMessageToOtherServer("006", config_MTL.SERVER_PORT_TOR, customerID, newEventID, newEventType);
						} else if (newEventID.substring(0, 3).equalsIgnoreCase("OTW")) {
							s += sendMessageToOtherServer("006", config_MTL.SERVER_PORT_OTW, customerID, newEventID, newEventType);
						} else {
						...
						if (s.contains("successfully!")) {
							...
							if (isLocalEvent(oldEventID)) {
								...
								// bookRecord.
								s += sendMessageToOtherServer("004", config_MTL.SERVER_PORT_TOR, oldEventID, oldEventType);
							} else if (oldEventID.substring(0, 3).equalsIgnoreCase("OTW")) {
								// customer have booked this eventID and remove from the other server's
								// bookRecord.
								s += sendMessageToOtherServer("004", config_MTL.SERVER_PORT_OTW, oldEventID, oldEventType);
							} 
	...
	public synchronized String addEvent(String eventID, String eventType, int bookingCapacity){
		...
		if (s.contains("successfully")) {
			config_MTL.LOGGER
					.info(config_MTL.MANAGER_ID + " add Event Record: " + getEventName(eventType) + " " + eventID + " "
							+ bookingCapacity + ". Request successfully completed. " + " Server response: " + s);
		} else {
			config_MTL.LOGGER.info(config_MTL.MANAGER_ID + " add Event Record: " + getEventName(eventType) + " "
	...
	public synchronized String removeEvent(String eventID, String eventType){
		...
		if (s.contains("successfully")) {
			...
			config_MTL.LOGGER.info(config_MTL.MANAGER_ID + " remove Event Record: " + getEventName(eventType) + " "
	...
	public String listEventAvailability(String eventType){
		...
		s = s.trim();
		config_MTL.LOGGER.info(config_MTL.MANAGER_ID + " list event availability: " + getEventName(eventType)
				+ ". Request successfully completed. " + " Server response: " + s);
		return s;
	}
	...
	public static void main(String[] args) {
		try {
			...
			Server_MTL addobj = new Server_MTL();
			addobj.setORB(orb);
			...
			Server_interface href = Server_interfaceHelper.narrow(ref);
	...
	public static void initLogger() {
		try {
			...
			config_MTL.FH = new FileHandler("Server_Side_Log/" + config_MTL.SERVER_NAME + ".log", true);

...
import ServerInterface.CommonInterfacePOA;
 ...
 * @Author: Rui
 * @Description:
 * @Date: Created in 4:45 PM 2019-07-06
 * @Modified by:
 */
public class MTLImplementationClass extends CommonInterfacePOA {

    public static LogRecorder mtlLogRecorder;
    // initial port number
    ...
    public MTLImplementationClass() throws Exception {
        super();
        mtlLogRecorder = new LogRecorder(1);
        HashMap<String, HashMap<String, EventDetails>> eventList =
                new HashMap();
        ...
        this.tsList = tsList;
//        EventDetails ed1 = new EventDetails(2);
//        EventDetails ed2 = new EventDetails(3);
//        EventDetails ed3 = new EventDetails(3);
//        this.conList.put("MTLM220619", ed1);
//        this.conList.put("MTLM230619", ed2);
//        this.conList.put("MTLM240619", ed3);
    }
    ...
    public void addEventHelper(
            HashMap<String, EventDetails> curList,
            String userID,
            String eventID,
            int eventType,
            int bookingCapacity) {
        EventDetails cur;
        try {
            mtlLogRecorder.writeLog(
                    mtlLogRecorder.paraConstructor(userID, "", eventType,
                            eventID, bookingCapacity),
                    Action.ADDEVENT,
                    true,
                    "true");
            // !!!need to be modied!
    ...
    public boolean addEvent(
        ...
        if (eventType == 1) {
            addEventHelper(this.conList, userID, eventID, eventType,
                    bookingCapacity);
            return true;
        } else if (eventType == 2) {
            addEventHelper(this.semList, userID, eventID, eventType,
                    bookingCapacity);
            return true;
        } else if (eventType == 3) {
            addEventHelper(this.tsList, userID, eventID, eventType,
                    bookingCapacity);
            return true;
        } else return false;
    }

    public boolean removeEventHelper(
            HashMap<String, EventDetails> curList, String userID,
            String eventID, int eventType) {
        if (curList.containsKey(eventID)) {
            curList.remove(eventID);
            try {
                mtlLogRecorder.writeLog(
                        mtlLogRecorder.paraConstructor(userID, "",
                                eventType, eventID, 0),
                        Action.REMOVEEVENT,
                        true,
                        "true");
    ...
    public String listEventAvailability(String userID, int eventType) {
        ...
        if (eventType == 1) {
            ...
            result += sendRequest(torUdpPort, 0, userID, eventType, "");
        }
        if (eventType == 2) {
    ...
    public int bookEventHelper(
            String customerID, String eventID,
            int eventType) {
        EventDetails cur;
        if (eventType == 1) {
            // conference
    ...
    public int bookEvent(String customerID, String eventID, int eventType) {
        ...
        if (tripleCheck == -1) {
            ...
            if (eventID.startsWith("MTL")) {
                ...
                rs = Integer.parseInt(sendRequest(otwUdpPort, 2, customerID
                        , eventType, eventID).trim());
            } else if (eventID.startsWith("TOR")) {
                rs = Integer.parseInt(sendRequest(torUdpPort, 2, customerID
                        , eventType, eventID).trim());
            }
            try {
                if (rs == 0)
                    mtlLogRecorder.writeLog(
                            mtlLogRecorder.paraConstructor("", customerID,
                                    eventType, eventID, 0),
                            Action.C_BOOKEVENT,
                            true,
                            String.valueOf(rs));
    ...
    public String getBookingSchedule(String customerID) {
        ...
        result += sendRequest(torUdpPort, 1, customerID, 4, "");
        try {
            mtlLogRecorder.writeLog(
                    mtlLogRecorder.paraConstructor("", customerID, 4, "", 0),
                    Action.M_GETBOOKINGSCHEDULE,
                    true,
                    result);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public int cancelEvent(String customerID, String eventID, int eventType) {
        ...
        if (eventID.startsWith("MTL")) {
            ...
            rs = Integer.parseInt(sendRequest(otwUdpPort, 3, customerID,
                    eventType, eventID).trim());
        } else if (eventID.startsWith("TOR")) {
            rs = Integer.parseInt(sendRequest(torUdpPort, 3, customerID,
                    eventType, eventID).trim());
        }
        return rs;
    }
    ...
    public int swapEvent(
        ...
        int checkRs = swapCheck(customerID, newEventID, newEventType,
                oldEventID, oldEventType);
        if (checkRs == -3) {
            try {
                mtlLogRecorder.writeLog(
                        mtlLogRecorder.paraConstructor("", customerID,
                                newEventType, newEventID, 0),
                        Action.C_SWAPEVENTS,
                        false,
                        String.valueOf(-2));
            } catch (IOException e) {
        ...
        if (checkRs == 0) {
            int rs = -100;
            if (newEventID.startsWith("MTL")) {
                rs = bookEventHelper(customerID, newEventID, newEventType);
            } else if (newEventID.startsWith("OTW")) {
                rs = Integer.parseInt(sendRequest(otwUdpPort, 2, customerID
                        , newEventType, newEventID).trim());
            } else if (newEventID.startsWith("TOR")) {
                rs = Integer.parseInt(sendRequest(torUdpPort, 2, customerID
                        , newEventType, newEventID).trim());
            }
            try {
                if (rs == 0) {
                    ...
                    int finalRs = rs - 2;
                    mtlLogRecorder.writeLog(mtlLogRecorder.paraConstructor(
                            "", customerID,
                            newEventType, newEventID, 0),
                            Action.C_SWAPEVENTS,
                            false,
                            String.valueOf(finalRs));
                    return finalRs;
                }
    ...
    private int swapCheck(
        ...
        for (int i = 0; i < rsArray.length; i++) {
            String[] temp = rsArray[i].split(" ");
            if (temp.length > 1) rsAry[i] = temp[1];
            else rsAry[i] = "";
        }
        customer.updateFromServer(rsAry[0] + rsAry[3] + rsAry[6], 2);
        customer.updateFromServer(rsAry[1] + rsAry[4] + rsAry[7], 1);
    ...
    private String sendRequest(
            int serverPort, int requestType, String userID, int eventType,
            String eventID) {
        String rs = "";
        DatagramSocket aSocket = null;
        String requestStr = requestType + "-" + userID + "-" + eventType +
                "-" + eventID;
        try {
            ...
            DatagramPacket request =
                    new DatagramPacket(message, requestStr.length(), aHost,
                            serverPort); // request
            // packet ready
            aSocket.send(request); // request sent out
            System.out.println(
                    "Request message sent from the client is : " + new String(request.getData()));

            byte[] buffer = new byte[1000]; // to store the received data, it
            // will be populated by what receive method returns
            DatagramPacket reply =
                    new DatagramPacket(buffer, buffer.length); // reply

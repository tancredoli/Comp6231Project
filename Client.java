...
public class Client {
	...
	if (userID.length() != 8) {
	    System.out.println("Sorry, the format of your input is not " + "correct!");
	    System.exit(0);
	}
	if (checkID(userID) == 0) {
	    try {
    ...
    private static void customerStart(String[] args, String userID, Scanner sc, LogRecorder clientRecorder)
	    throws IOException {
	...
	while (true) {
	    System.out.println("-----------------------------------------------------------------------------------");
	    System.out.println("Please choose the option as follow(input " + "the character in brackets to continue):");
		...
		if (oldEventType < 0) {
		    System.out.println("Sorry, the old eventType you input " + "is " + "not correct.");
		    continue;
		}
		...
		if (newEventType < 0) {
		    System.out.println("Sorry, the new eventType you input " + "is " + "not correct.");
		    continue;
		}
		...
		if (!customer.checkValid(newEventID)) {
		    System.out.println("Sorry, the new eventID you input is" + " incorrect.");
		    continue;
		}
		int rs = obj.swapEvent(userID, newEventID, newEventType, oldEventID, oldEventType);
		if (rs == 0) {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, newEventType, newEventID, 0),
			    Action.C_SWAPEVENTS, true, String.valueOf(rs));
		    System.out.println("Congratulation, you have " + "successfully swap the events!");
		}
		if (rs == -1) {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, newEventType, newEventID, 0),
		...
		if (rs == -2) {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, newEventType, newEventID, 0),
			    Action.C_SWAPEVENTS, false, String.valueOf(rs));
		    System.out.println("Sorry, the event you swap from you " + "did't book before!");
		}
		if (rs == -3) {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, newEventType, newEventID, 0),
			    Action.C_SWAPEVENTS, false, String.valueOf(rs));
		    System.out.println("Sorry, the event you swap to " + "doesn't exist!");
		}
		if (rs == -4) {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, newEventType, newEventID, 0),
			    Action.C_SWAPEVENTS, false, String.valueOf(rs));
		    System.out.println("Sorry, the event you swap to " + "has already been booked by yourself");
		}
		if (rs == -5) {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, newEventType, newEventID, 0),
			    Action.C_SWAPEVENTS, false, String.valueOf(rs));
		    System.out.println("Sorry, the event you swap to " + "has no availability!");
		}
		if (rs == -6) {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, newEventType, newEventID, 0),
			    Action.C_SWAPEVENTS, false, String.valueOf(rs));
		    System.out.println("Sorry, the event you swap to " + "has a conflict with your booked events!");
		}
		if (rs == -7) {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, newEventType, newEventID, 0),
			    Action.C_SWAPEVENTS, false, String.valueOf(rs));
		    System.out.println("You can't book more than three " + "events outside your city!");
		}
		...
		if (eventType < 0) {
		    System.out.println("Sorry, the eventType you input is " + "not correct.");
		    continue;
		}
		System.out.println("Please input the eventID:");
		String eventID = sc.nextLine().toUpperCase();
		if (!customer.checkValid(eventID)) {
		    System.out.println("Sorry, the eventID you input is " + "incorrect.");
		    continue;
		}
		int rs = obj.bookEvent(userID, eventID, eventType);
		if (rs == 0) {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, eventType, eventID, 0),
			    Action.C_BOOKEVENT, true, "0");
		    System.out.println("Congratulation, you have " + "successfully booked " + "this event!");
		} else if (rs == -1) {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, eventType, eventID, 0),
			    Action.C_BOOKEVENT, false, "-1");
		    System.out.println("Sorry, the event you booked doesn't" + " exist.");
		} else if (rs == -2) {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, eventType, eventID, 0),
		    ...
		    System.out.println("Sorry, you can not book events " + "which are at same time slot.");
		} else if (rs == -5) {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, eventType, eventID, 0),
			    Action.C_BOOKEVENT, false, "-5");
		    System.out.println("Sorry, you can book at most three " + "events in one month outside your city!");
		}
		continue;
	    }
		...
		clientRecorder.writeNorLog(clientRecorder.paraConstructor("", userID, 4, "", 0),
    ...
    private static void managerStart(String[] args, String userID, Scanner sc, LogRecorder clientRecorder)
	    throws IOException {
	CommonInterface obj = getRemoteObj(args, userID);
	System.out.println("Welcome back, " + userID + "!");
	while (true) {
	    ...
	    System.out.println("3. (L)ist all the events of a certain event" + " type.");
	    ...
	    System.out.println("0. Tada! Use (test) to output a booked list" + " " + "of a certain event.");
		...
		if (isValid != 1) {
		    System.out.println("Sorry, the customerID you input is " + "not correct!");
		    continue;
		}
		if (!checkManager(userID, customerID)) {
		    System.out.println("Sorry, you don't have the " + "authorization!");
		    continue;
		}
		System.out.println("Please input the old eventType:");
		int oldEventType = checkEventType(sc.nextLine());
		...
		int rs = obj.swapEvent(customerID, newEventID, newEventType, oldEventID, oldEventType);
		if (rs == 0) {
		    clientRecorder.writeNorLog(
			    clientRecorder.paraConstructor(userID, customerID, newEventType, newEventID, 0),
		...
		clientRecorder.writeNorLog(clientRecorder.paraConstructor(userID, customerID, 4, "", 0),
		...
		if (!checkEventID(eventID, userID.substring(0, 3))) {
		    System.out.println("Sorry, the eventID you input is " + "not correct.");
		    continue;
		}
		System.out.println("Please input the bookingCapacity:");
		int bookingCapacity = Integer.parseInt(sc.nextLine());
		if (bookingCapacity < 0) {
		    System.out.println("Sorry, the format of " + "bookingCapacity is incorrect");
		    continue;
		}
		...
		if (obj.removeEvent(userID, eventID, eventType)) {
		    ...
		    System.out.println("The event has been successfully " + "removed!");
		} else {
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor(userID, "", eventType, eventID, 0),
			    Action.REMOVEEVENT, false, "false");
		    System.out.println("The event you want to remove does " + "not exists.");
		}
		...
		if (rs.length() != 0) {
			...
			rs = "Trade Shows: " + rs;
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor(userID, "", eventType, "", 0),
			    Action.LISTEVENTAVAILABILITY, true, rs);
		    System.out.println(rs);
		} else {
		    System.out.println("Sorry, there is no records of this " + "type in database.");
		    clientRecorder.writeNorLog(clientRecorder.paraConstructor(userID, "", eventType, "", 0),
			    Action.LISTEVENTAVAILABILITY, true, rs);
		}
    ...
    public static CommonInterface getRemoteObj(String[] args, String userID) {
	...
	try {
	    ...
	    obj = CommonInterfaceHelper.narrow(ncRef.resolve_str("frontend"));

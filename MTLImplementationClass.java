 ...
 * @Modified by:
 */
public class MTLImplementationClass {
	...
	// EventDetails ed1 = new EventDetails(2, booklist1);
	// EventDetails ed2 = new EventDetails(3, booklist1);
	...
	// this.semList.put("MTLE190324", ed3);
	// this.tsList.put("MTLa190324", ed1);
    ...
    public void addEventHelper(HashMap<String, EventDetails> curList, String userID, String eventID, int eventType,
	    int bookingCapacity) {
	EventDetails cur;
	mtlLogRecorder.writeNorLog(mtlLogRecorder.paraConstructor(userID, "", eventType, eventID, bookingCapacity),
	...
	if (curList.containsKey(eventID)) {
	    curList.remove(eventID);
	    mtlLogRecorder.writeNorLog(mtlLogRecorder.paraConstructor(userID, "", eventType, eventID, 0),
	...
	mtlLogRecorder.writeNorLog(mtlLogRecorder.paraConstructor(userID, "", eventType, "", 0),
	...
	if (tripleCheck == -1) {
		...
		rs = Integer.parseInt(sendRequest(torUdpPort, 2, customerID, eventType, eventID).trim());
	    }
	    if (rs == 0)
		mtlLogRecorder.writeNorLog(mtlLogRecorder.paraConstructor("", customerID, eventType, eventID, 0),
	...
	mtlLogRecorder.writeNorLog(mtlLogRecorder.paraConstructor("", customerID, 4, "", 0),
	...
	if (eventID.startsWith("MTL")) {
	    ...
	    rs = Integer.parseInt(sendRequest(torUdpPort, 3, customerID, eventType, eventID).trim());
	}
	return rs;
    }
	...
	if (checkRs == -3) {
	    mtlLogRecorder.writeNorLog(mtlLogRecorder.paraConstructor("", customerID, newEventType, newEventID, 0),
	...
	if (checkRs == 0) {
		...
		rs = Integer.parseInt(sendRequest(torUdpPort, 2, customerID, newEventType, newEventID).trim());
	    }
	    if (rs == 0) {
	...
	try {
	    ...
	    DatagramPacket reply = new DatagramPacket(buffer, buffer.length); // reply
